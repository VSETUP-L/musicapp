<template>
  <div id='userGZ'>
    <div v-show="GZlist.length<=0">暂无关注</div>
    <div v-show="!GZlist.length<=0">
    <h2 style="margin: 10px 0 20px  0;">{{otheruser.profile?otheruser.profile.nickname:''}}的关注</h2>
    <li class="box" v-for="(item, index) in GZlist" :key="index" >
          <img
...
export default {
  ...
  methods: {
    ...
    getGZ(){
        ...
        let url = `/user/follows?uid=${this.userid}`;
